...
def find_best_params(X, y, params_grid, record_fi):
    ...
    with open(record_fi, "a+") as fout:
        ...
        res = Parallel(n_jobs=-1)(delayed(loo_validation)(params_['tree_num'],
                                                   params_["max_depth"],
    ...
    acc_record = acc_record.sort_values(by=[ "specificity", "sensitivity", 'acc'], ascending=False)
...
class my_RF_run:
    ...
    def testing(self):
        ...
        pd_eval = pd.DataFrame(columns=["topk", "loo_acc", "train_acc", "sensitivity", "specificity"])
        for topk in topk_for_eval:
            ...
            acc, train_acc, sensitivity, specificity = eval_topk_RF_feature(X_imp, y, new_feature_name,
                                                                            best_tree_num,
                                                                            best_max_depth, best_min_samples_split,
                                                                            best_class_weight, fout)
