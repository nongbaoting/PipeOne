...
def find_best_params(X, y, params_grid, record_fi):
    ...
    acc_record = pd.DataFrame(columns=["tree_num","max_depth","min_samples_split","class_weight","acc",  "sensitivity", "specificity"])
    with open(record_fi, "a+") as fout:
        ...
        res = Parallel(n_jobs= 24 )(delayed(loo_validation)(params_['tree_num'],
                                                   params_["max_depth"],
                                                   params_['min_samples_split'], params_["class_weight"], X, y) for params_ in all_params)
        acc_all, train_acc_all, sensitivity_all, specificity_all, _ = zip(*res)
        for acc, train_acc, sensitivity, specificity,params in zip(acc_all, train_acc_all, sensitivity_all, specificity_all,all_params):
            ...
            line = "In tree_num={:d}, max_depth={:d}, min_samples_split={:d}, class_weigth={:}, train_acc={:.4f}, loo_acc={:.4f}, sensitivity={:.4f}, specificity={:.4f}".format(
                params["tree_num"], params["max_depth"], params["min_samples_split"], params["class_weight"], train_acc, acc, sensitivity,
                specificity)
            #print(line)
            fout.write(line + "\n")
    ...
    acc_record = acc_record.sort_values(by=["specificity", "sensitivity", 'acc',"max_depth","min_samples_split","tree_num"],
    ...
    best_class_weight = acc_record.loc[0, "class_weight"]
    ...
    print(best_tree_num, best_max_depth, best_min_samples_split,best_class_weight,  best_acc)
    return best_tree_num, best_max_depth, best_min_samples_split, best_class_weight, best_acc


def save_feature_importance(X, y, feature_name, tree_num, max_depth, min_samples_split, class_weight, fout):
    feature_importance = get_feature_importance(tree_num, max_depth, min_samples_split, class_weight, X, y)
...
best_params_record = {"data_dir": [], "tree_num": [],
                      "max_depth": [], "min_samples_split": [], "class_weight": [],
                      "best_loo_acc": []}
...
for i in range(len(data_assemble)):
    ...
    testX, testy, testfeature_name = load_data(test_dat_dir)
    ...
    best_params_record["class_weight"].append(best_class_weight)
    ...
    feature_imp = save_feature_importance(X, y, feature_name,
                            ...
                            best_min_samples_split, best_class_weight, feature_imp_fi[i])

    # eval classification ability of top k features computed by RF
    # feature_imp = load_feature_imp(feature_imp_fi[i])
    pd_eval = pd.DataFrame(columns=[ "topk", "loo_acc", "train_acc", "sensitivity",
                                     "specificity","test_sensitivity", "test_specificity"])

    for topk in topk_for_eval:
        X_imp, new_feature_name, X_imp_idx = select_top_imp_feature(X, feature_imp, feature_name, topk)
        fout = tdir + "feature(%s)_importance_top%d_for_retraining.csv" \
               % ([data_assemble[i], "all"][data_assemble[i] == "proc"], topk)
        acc, train_acc,sensitivity, specificity, test_sensitivity, test_specificity = eval_topk_RF_feature(testX, testy, X_imp, y,new_feature_name, X_imp_idx, best_tree_num,
                                              best_max_depth, best_min_samples_split, best_class_weight, fout)

        pd_eval_ = pd.DataFrame({"topk":[topk], "loo_acc": [acc], "train_acc": [train_acc],
                                 "sensitivity":[sensitivity],     "specificity":[specificity],
                                 "test_sensitivity": [test_sensitivity], "test_specificity": [test_specificity] } )
        pd_eval = pd_eval.append(pd_eval_, ignore_index=True)
    ...
    pd_eval.to_csv(os.path.join(tdir, "eval_RF_topk_features.csv"), index=False)
