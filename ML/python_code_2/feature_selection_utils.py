...
def loo_validation(tree_num, max_depth, min_samples_split, class_weight, X, y):
    loo = LeaveOneOut()
    count = 0
    y_predict_lst = []
    y_test_lst = []
    for train_index, test_index in loo.split(X):
        ...
        clf = RandomForestClassifier(n_estimators=tree_num,
                                     ...
                                     class_weight=class_weight, random_state=0, n_jobs=4)
    ...
    return acc, train_acc, test_sensitivity, test_specificity, clf_train


def get_feature_importance(tree_num, max_depth, min_samples_split, class_weight, X, y):
    clf = RandomForestClassifier(n_estimators=tree_num,
                                 max_depth=max_depth,
                                 min_samples_split=min_samples_split, class_weight=class_weight, random_state=0)
    clf.fit(X, y)
...
def load_data(data_dir):
    ...
    for fi in data_fi:
        if "clustering" in fi:
            ...
            data = pd.read_csv( os.path.join(data_dir, fi) , header=0, index_col=0)
...
def select_top_imp_feature(X, feature_imp, feature_name, topk):
    ...
    return X_imp, new_feature_name, sort_idx[0:topk]


def get_test_SS(model_trained, all_testX, all_testy, sort_index):
    testX_sub, testy_sub, = all_testX[:, sort_index], all_testy,
    testX_sub = np.nan_to_num(testX_sub)
    y_test_pred = model_trained.predict(testX_sub)
    test_sensitivity, test_specificity = sensitivity_specificity(testy_sub, y_test_pred)
    return  test_sensitivity, test_specificity


def eval_topk_RF_feature(testX, testy,  X, y,  feature_name, X_imp_idx, tree_num, max_depth, min_samples_split, class_weight, fout):

    # X_imp, new_feature_name = select_top_imp_feature(X, feature_imp, feature_name, topk)
    acc, train_acc, sensitivity, specificity, model_trained = loo_validation(tree_num, max_depth, min_samples_split, class_weight, X, y)
    feature_importance = get_feature_importance(tree_num, max_depth, min_samples_split, class_weight, X, y)
    test_sensitivity, test_specificity = get_test_SS(model_trained, testX, testy, X_imp_idx)
    ...
    return acc, train_acc,sensitivity, specificity,test_sensitivity, test_specificity
